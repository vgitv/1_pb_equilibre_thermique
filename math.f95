...
contains
    ...
    function seq(a, b, h)
        ...
        ! return
        double precision, dimension(:), allocatable :: seq
    ...
    function evaluate(fc, vec)
        ...
        ! return
        double precision, dimension(size(vec)) :: evaluate
    ...
    subroutine saveSol(x, sol, fichier)
        ...
        ! parametres
        double precision, dimension(:) :: x, sol
    ...
    subroutine plu(n, A, P, L, U)
        ...
        double precision, dimension(n, n), intent(out) :: P, L, U

        ! variables locales
        integer :: i, j, k
        double precision, dimension(n) :: temp
        double precision :: maxk
    ...
    subroutine descenteRemontee(n, b, P, L, U, x)
        ...
        double precision, dimension(n), intent(in) :: b! second menbre
        double precision, dimension(n, n), intent(in) :: P, L, U! issu décomposition plu
        double precision, dimension(n), intent(out) :: x! solution

        ! variables lodales
        integer :: i
        double precision, dimension(n) :: Pb, verif
        double precision :: temp
    ...
    subroutine linSolve(method, n, A, b, x)
        ...
        integer, intent(in) :: n! dimension de l'espace
        double precision, dimension(n, n), intent(in) :: A! application linéaire
        double precision, dimension(n), intent(in) :: b! second membre
        double precision, dimension(n), intent(out) :: x! solution

        ! variables locales
        double precision, dimension(n, n) :: P, L, U
    ...
    subroutine trapeze(x, fx, somme)
        ...
        ! paramètres
        double precision, dimension(:), intent(in) :: x, fx
        double precision, intent(out) :: somme
